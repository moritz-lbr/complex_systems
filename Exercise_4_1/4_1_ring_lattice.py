...
k = 2   # Number of nearest neighbors on each side
...
# Add edges based on the ring lattice structure
for i in range(n):
    for j in range(1, k+1):
        G.add_edge(i, (i+j) % n)  # Connect node i to node (i+j) mod n (circular)

# Plot the graph
nx.draw(G, with_labels=True, node_color='lightblue', edge_color='gray', node_size=700)
plt.title(f"Ring Lattice with {2*k}-Nearest Neighbors and {n} nodes")
plt.show()
